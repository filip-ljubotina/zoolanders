...
function ChooseSubjectView({cardData, onSubmit}) {
    ...
    try {
        const responseBreeds = await ApiService.get(`/wildTrack/animal/findAllBreedsByStation/${cardData.locationName}`);
    ...
    const putData = async () => {
        try {
            await ApiService.put(`/wildTrack/researcher/putMapViewCriteria/${cardData.actionId}`, subjectMapView);
        } catch (error) {
            console.error('Error fetching table data:', error);
        }
    };
    ...
    return (
        ...
        <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
            ...
            <Select id="subject" name="subject" value={subject} onChange={handleChange} required>
                <SelectOption key="individual" value="individual">Jedinka</SelectOption>
                <SelectOption key="breed" value="breed">Vrsta</SelectOption>
            </Select>

            {subject === 'breed' &&  (<DialogContentText>
                Odaberite vrste životinja za proučavanje
            </DialogContentText>)}
            {subject === 'breed' && (<FormGroup>
                {allBreeds.map((breed) => (
                    <FormControlLabel
                        key={breed}
                        control={
            ...
            {subject === 'individual' && (<DialogContentText>
                Odaberite životinje za proučavanje
            </DialogContentText>)}
            {subject === 'individual' && (<FormGroup>
                    ...
                    <FormControlLabel
                        ...
                        control={
                            <Checkbox
                                ...
                                onChange={() => handleCheckboxAnimalsChange(animal.animalId)}
                            />
                        }
