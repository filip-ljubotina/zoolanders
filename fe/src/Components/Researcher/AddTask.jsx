    ...
    if (isRoutingActive == true) {
      routingControlRef.current = L.Routing.control({
        ...
        createMarker: (i, waypoint, n) => {
          const iconUrl =
            i === 0 ? searcher_icon_png : i === n - 1 ? flag_icon : circle_icon;
  ...
  const postData = async () => {
    try {
      await ApiService.post(
        `/wildTrack/researcher/postNewTask/${cardData.actionId}`,
        newTask
      );
    } catch (error) {
  ...
  return (
    <React.Fragment>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <DialogContentText>Što je zadatak tragača?</DialogContentText>
          <Select
            id="taskToDo"
            name="taskToDo"
            value={taskToDo}
            ...
            required
          >
            <SelectOption key="camera" value="camera">
              Postavljanje kamere
            </SelectOption>
            <SelectOption key="tracker" value="tracker">
              Postavljanje uređaja za praćenje
            </SelectOption>
          </Select>
          ...
          >
            ...
            {isRoutingActive === false && (
              <Polyline
                positions={extractLatLng(waypointsRef.current)}
                color="blue"
              />
            )}
          </MapContainer>
